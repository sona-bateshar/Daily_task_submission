...
interface TagInputProps {
  label?: string;
  placeholder?: string;
  value?: string[]; // Add value prop for form integration
  onChange?: (tags: string[]) => void; // Add onChange for form integration
  initialTags?: string[];
...
const TagInput: React.FC<TagInputProps> = ({
  ...
  value, // Controlled value from form
  ...
  // Determine which tags to use (controlled vs uncontrolled)
  ...
  // Update tags function that handles both controlled and uncontrolled modes
  const updateTags = (newTags: string[]) => {
    // If controlled (form integration)
    if (onChange) {
      onChange(newTags);
    } else {
      // If uncontrolled (internal state)
      setInternalTags(newTags);
    }
    
    // Call external callback if provided (for additional side effects)
    if (onTagsChange) {
