...
from .permissions import TaskModelPermission, CommentModelPermission
...
class CommentViewSet(viewsets.ModelViewSet):
    permission_classes = [IsAuthenticated, CommentModelPermission ]
    ...
    def get_queryset(self):
        ...
        handling the new, more complex filtering logic.
        """
        task_id = self.request.query_params.get("task__id")
        if not task_id:
            raise ValidationError({"task": "Query parameter 'task__id' is required."})

        
        # queryset = Comment.objects.filter(task=task_id)
        queryset = Comment.objects.filter(**self.request.GET.dict())
